...
export class SellerAgent {
  ...
  async fetch(request) {
    try {
      ...
      if (!(request.method === 'POST' && url.pathname === '/execute')) throw new HttpError(404, 'Not found')
  ...
  async #execute(body) {
    ...
    tx.signIdx(hex.decode(this.env.SELLING_WALLET_PRIVATE_KEY), INSCRIPTION_INPUT_INDEX)
    tx.finalize()
  ...
  async #validateMempoolAcceptance(tx) {
    ...
    if (!mempoolTest.allowed) throw new HttpError(400, `Transaction rejected by mempool: ${mempoolTest.rejectReason ?? 'unknown'}`, 'mempool_reject')
